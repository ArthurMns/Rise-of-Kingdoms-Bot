    ...
    def validate_cmd(value, action_type):
        if action_type == "1":
            ...
            if len(value) > 1 and value[0] == "0":
                return False
        setattr(
            app.bot_config, attr_name, int(value if value != "" else str(def_value))
        )
...
# Mystery Merchant
mystery_merchant_checkbox = checkbox_fn_creator(
    "enableMysteryMerchant", "Use resource buy item in Mystery Merchant"
)
...
# In city
collecting_checkbox = checkbox_fn_creator(
    "enableCollecting", "Collecting resource, troops, and help alliance"
)
...
open_free_chest_in_tavern = checkbox_fn_creator(
    "enableTavern", "Open free chest in tavern"
)

training = checkbox_fn_creator("enableTraining", "Auto upgrade and train troops")

train_barracks = train_fn_creator(
    "Barracks:", "trainBarracksTrainingLevel", "trainBarracksUpgradeLevel"
)

train_archery_range = train_fn_creator(
    "Archery:", "trainArcheryRangeTrainingLevel", "trainArcheryRangeUpgradeLevel"
)

train_stable = train_fn_creator(
    "Stable:", "trainStableTrainingLevel", "trainStableUpgradeLevel"
)

train_siege = train_fn_creator(
    "Siege:", "trainSiegeWorkshopTrainingLevel", "trainSiegeWorkshopUpgradeLevel"
)

# other
daily_vip_point_and_chest = checkbox_fn_creator(
    "enableVipClaimChest", "Claim daily vip point and chest"
)
vip_do_round = entry_int_fn_creator("vipDoRound", "Execute at every", "round")

claim_quest_checkbox = checkbox_fn_creator(
    "claimQuests", "Claim quests and daily objectives"
)
quest_do_round = entry_int_fn_creator("questDoRound", "Execute at every", "round")

alliance_action_checkbox = checkbox_fn_creator(
    "allianceAction", "Collecting allied resource, gifts, and donate technology"
)
alliance_do_round = entry_int_fn_creator("allianceDoRound", "Execute at every", "round")

get_alliance_member_info = checkbox_fn_creator(
    "enableGetInfoAllianceMember", "Get info of alliance member"
)

# Outside

attack_barbarians_checkbox = checkbox_fn_creator(
    "attackBarbarians", "Attack Barbarians"
)
hold_position_checkbox = checkbox_fn_creator(
    "holdPosition", "Hold Position After Attack"
)
heal_troops_checkbox = checkbox_fn_creator(
    "healTroopsBeforeAttack", "Heal Troops Before Attack"
)
use_daily_ap_checkbox = checkbox_fn_creator(
    "useDailyAPRecovery", "Use Daily AP Recovery"
)
use_normal_ap_checkbox = checkbox_fn_creator(
    "useNormalAPRecovery", "Use Normal AP Recovery"
)
barbarians_base_level_entry = entry_int_fn_creator(
    "barbariansBaseLevel", "Base Level(normal/kvk):"
)
barbarians_min_level_entry = entry_int_fn_creator(
    "barbariansMinLevel", "Minimum attack Level:"
)
barbarians_max_level_entry = entry_int_fn_creator(
    "barbariansMaxLevel", "Maximum attack Level:"
)
...
resource_no_secondery_commander = checkbox_fn_creator(
    "gatherResourceNoSecondaryCommander", "Not secondary commader"
)
use_gathering_boosts = checkbox_fn_creator("useGatheringBoosts", "Use gathering boosts")
hold_one_query_space_checkbox = checkbox_fn_creator(
    "holdOneQuerySpace", "Hold space for attack barbarians"
)

enable_scout_checkbox = checkbox_fn_creator("enableScout", "Enable explore")
enable_Investigation_checkbox = checkbox_fn_creator(
    "enableInvestigation", "Investigate Cave, Village"
)

enable_sunset_canyon_checkbox = checkbox_fn_creator(
    "enableSunsetCanyon", "Enable Sunset Canyon"
)
enable_lost_canyon_checkbox = checkbox_fn_creator(
    "enableLostCanyon", "Enable Lost Canyon"
)
...
use_items_daily_rss = checkbox_fn_creator(
    "useItemsDailyRss", "Use 5 Level1 Resource Packs for Daily Quest"
)
...
def resource_ratio(app, parent):
    ...
    for col in range(4):
        ...
        entry.config(
            validate="key",
            validatecommand=(frame.register(creator(attr_names[col])), "%P", "%d"),
        )
